...
```
Select all Customers with the id of 2:
```sql
...
BETWEEN 22 AND 40;
```
Select all cities ending in the letter 'n':
```sql
SELECT * FROM Customers
WHERE City LIKE '%n';
```
_% is a wildcard similar to the regex *_

Select all cities containing the letter 'n':
```sql
SELECT * FROM Customers
WHERE City LIKE '%n%';
```
Select all cities not containing the letter 'n':
```sql
SELECT * FROM Customers
...
GROUP BY Age;
```
Select all Customers where the count for Age >= 2:
```sql
SELECT Age, COUNT(Age) 
FROM Customers
GROUP BY Age
